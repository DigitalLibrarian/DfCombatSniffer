...
--        <pointer name="curse" type-name='wound_curse_info'/>
	...
	for i=1, numParts, 1 do
		...
		Append("EDGED_CURVE_PERC: " .. part.edged_curve_perc)
...
--	for kk,vv in pairs(v.layers) do
		...
		--if(tisdata.id == tissue_id) then
			...
			Append("IMPACT_YIELD: " .. matdata.yield.IMPACT)
			Append("IMPACT_FRACTURE: " .. matdata.fracture.IMPACT)
			Append("IMPACT_STRAIN_AT_YIELD: " .. matdata.strain_at_yield.IMPACT)
			Append("IMPACT_DENT_COST: " .. vbca)
			Append("IMPACT_FRACTURE_START_COST: " .. vbcb)
			Append("IMPACT_FRACTURE_COMPLETE_COST: " .. vbcc)
